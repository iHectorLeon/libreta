...
export class UserService{
  ...
  singUp(usertologin):Observable<any>{
    ...
    return this.http.post(this.url+'login', json, headers)//.map(res=>res.json());
  }
  ...
  */
  getUser(username):Observable<any>{
    ...
    return this.http.get(this.url+'api/v1/user/getdetails?name='+username,headers)//.map(res=>res.json());
  }
