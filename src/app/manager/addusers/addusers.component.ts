...
export class AddusersComponent implements OnInit {
          ...
          if(this.validateEmails(id.Correo_electronico)){
            if(this.csvResult.length>0){
              if(this.csvResult.find(res => res.value.Correo_electronico == id.Correo_electronico)){
                let result = {'value':id,"flag":'secondary', "result":"Registro repetido"}
                this.csvResult.push(result);
                this.resultfind++;
              }else{
                let result = {'value':id,"flag":'success', "result":"Registro correcto"}
                this.csvResult.push(result);
            ...
            let result = {'value':id,"flag":'danger', "result":"Correo no valido"}
            this.csvResult.push(result);
            this.resultfalse++;
          }
        }else{
          let result = {'value':id,"flag":'danger', 'result':"El registro no cumple con los estandares de información"}
          this.csvResult.push(result);
          this.resultfalse++;
        }
      }
  ...
  */
  public processFiles(){
    ...
    for(let idstudent of this.studentscsvfile){
    ...
    this.closeModal();
    console.log(this.studenstTemp);
    console.log(this.studentsforgrouptmp);
  }
    ...
    this.managerServices.getRequestFinder(this.numberrequest).subscribe(
      data=>{
        ...
        if(valuetemp2.students.length > 0){
          ...
          let contador = 0;
          for(let id of this.studentsforgrouptmp){
